...
export default function PaymentGroup({
  group, checked, onToggle, onReset, values, onValueChange, lastReset,
}) {
  ...
  const totalSum = group.items.reduce((s, item) => s + (values[item.id] || 0), 0);
  const paidSum = group.items.reduce(
    (s, item) => s + (checked[item.id] ? values[item.id] || 0 : 0), 0
  );
